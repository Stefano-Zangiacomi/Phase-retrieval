function [Bspec,waxis] = Calbispectrum2D(y,  nfft, wind, nsamp, overlap, Initial_Bspec)
...
% ---------------- accumulate triple products ----------------------
    ...
    for krec = 1:nrecs
        xseg   = y(locseg);
%         Xf     = fftshift(fft(ifftshift(xseg-mean(xseg)), nfft))/nsamp;   %1
        Xf     = fft(xseg-mean(xseg), nfft)/nsamp;    %2
        CXf    = conj(Xf);
        Bspec  = Bspec + (Xf * Xf.') .* ...
